function WebOffice2015() {
    ...
    this.FileType = ".doc"; // FileType:文档类型 .doc .xls .wps
    this.obj; // obj：object 对象
    this.Status = "";               //Status：状态信息
	this.WebUrl = "http://www.kinggrid.com/"
    ...
    */
            ...
            if (httpclient.Send()) {
                ...
                if (this.hiddenSaveLocal(httpclient, this, false, false)) {//下载成功时
				 if(this.OpenLocalFile(this.FilePath+this.FileName)==0){ //打开本地磁盘文件
					...
					//	 window.close(); 
               // }
			this.obj.FuncExtModule().Alert("打开文档时未知错误！请尝试重新打开");
					 return false;
            }
	        }
		 }
    ...
    this.getOfficeVersion = function () {
        ...
        try {
    		if(this.setVersion == -1){
    			 getVersion = parseFloat(this.obj.Application().Version());
    ...
    this.WebPageCode = function () {
        //var FunctionString = "ActiveObject.ActiveDocument.Application.Dialogs().Item(294).Show()";
        //this.blnIE() ? this.obj.ActiveDocument().Application().Dialogs().Item(294).Show() : this.ExecuteScript("WebPageCode", FunctionString);
    ...
    */
    ...
    this.WebOpenBookMarks = function(){	
    	...
    	//this.blnIE()?this.obj.ActiveDocument().Application().Dialogs().Item(168).Show():this.ExecuteScript("WebPageCode",FunctionString);
  ...
  //光标处添加书签
    ...
    this.WebFindBookMarks = function(BMarksName){
		var range = this.obj.ActiveDocument().Application().Selection().Range();
		var bookmark = this.obj.ActiveDocument().Bookmarks().Item(BMarksName);
   ...
   //删除书签
    ...
    this.InsertImageByBookMark = function () {
        ...
        if(this.obj.ActiveDocument().Application().Selection().InlineShapes().AddPicture("D:\\1.png",false,true) !== undefined)
		{
			...
			this.obj.FuncExtModule().Alert("请确保图片 D:\\1.png 存在");
		}
		return false;
    }
    ...
    this.VBASetBookmarks = function (vbmName, vbmValue) {
        try {
            if(this.obj.ActiveDocument().BookMarks().Exists(vbmName)){
                var mBookRange = this.obj.ActiveDocument().Bookmarks().Item(vbmName).Range();
   ...
   // 	 this.obj.ActiveDocument().ActiveWindow().View().ShowFieldCodes = true;
     ...
     this.WebAreaProtect = function(BMarksName){
	    if(!this.obj.ActiveDocument().BookMarks().Exists(BMarksName)){//判断是否存在该书签 
	    	this.obj.FuncExtModule().Alert("文件内没有名称为'"+BMarksName+"'的书签 ,请先添加书签!");
	    	return;
		}
    	...
    	this.obj.ActiveDocument().Application().Selection().MoveLeft(Unit = 1, Count = 1,0);	
     ...
     this.WebAreaUnprotect = function(BMarksName){
	 	...
	 	try{
	 		...
	 		this.obj.FuncExtModule().Alert("不存在该书签!");
	 		return;
	 	}
     }
  ...
  //手写签批
    ...
    */
    ...
    this.WebSaveHtml = function(){
	  ...
	  {
		  this.obj.FuncExtModule().Alert("保存Html成功!\n保存路径:  "+"D:\\"+this.RecordID + ".html");
	  }
    }
	//执行宏
